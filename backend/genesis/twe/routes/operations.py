...
def get_operation(operation_id):
    with current_app.config["TWE_DB"].connect() as conn:
        ...
        if operation["capability"] == "instance.provision" and operation["status"] in {"requested", "queued", "executing", "verifying"}:
            reconcile_instance(conn, current_app.config["TWE_CONFIG"], operation["game_instance_id"])
            operation = fetch_one(
                conn,
                """
                SELECT so.id::text, so.game_instance_id::text, so.requested_by::text, so.capability,
